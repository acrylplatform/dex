...
import com.acrylplatform.api.http.assets.{SignedIssueV2Request, SignedMassTransferRequest}
import com.acrylplatform.common.utils.EitherExt2
import com.acrylplatform.it.api.Transaction
import com.acrylplatform.it.util.GlobalTimer
import com.acrylplatform.dexgen.cli.ScoptImplicits
import com.acrylplatform.dexgen.config.FicusImplicits
...
object DexGenApp extends App with ScoptImplicits with FicusImplicits with EnumerationReader {
  ...
  parser.parse(args, defaultConfig) match {
    ...
    case Some(finalConfig) =>
      ...
      val transfers = Seq(
        massTransfer(endpoint, finalConfig.richAccounts.head, finalConfig.validAccounts, Acryl +: tradingAssets.dropRight(2)),
        massTransfer(endpoint, finalConfig.richAccounts.head, finalConfig.fakeAccounts, Acryl +: tradingAssets.takeRight(2))
      )
