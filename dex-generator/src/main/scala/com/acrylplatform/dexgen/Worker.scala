...
import cats.Show
import com.acrylplatform.account.{AddressOrAlias, KeyPair, PublicKey}
import com.acrylplatform.api.http.assets.SignedTransferV1Request
import com.acrylplatform.common.state.ByteStr
import com.acrylplatform.crypto
import com.acrylplatform.dexgen.Worker._
import com.acrylplatform.dexgen.utils.{ApiRequests, GenOrderType}
import com.acrylplatform.it.api.{MatcherResponse, MatcherStatusResponse, OrderbookHistory, Transaction, UnexpectedStatusCodeException}
import com.acrylplatform.it.util._
import com.acrylplatform.dex.AssetPairBuilder
import com.acrylplatform.dex.api.CancelOrderRequest
...
class Worker(workerSettings: Settings,
  ...
  def transfer(i: Long, sender: KeyPair, assetId: Asset, recipient: KeyPair, halfBalance: Boolean)(
    ...
    to(endpoint).balance(sender.address, assetId).flatMap { balance =>
      ...
      val transferAmount = assetId.fold(halfAmount - 0.001.acryl)(_ => halfAmount)
      ...
      TransferTransactionV1.selfSigned(assetId,
        ...
        case Right(txRequest) =>
          log.info(
            s"[$tag] ${assetId.compatId.fold("Acryl")(_.toString)} balance of ${sender.address}: $balance, sending $transferAmount to ${recipient.address}")
  ...
  def send(i: Long, orderType: GenOrderType.Value): Future[Any] = {
    ...
    val pair = createAssetPair(
      randomFrom(tradingAssets.dropRight(2).dropRight(tradingAssetsSize / 2)).getOrElse(Acryl),
      randomFrom(tradingAssets.dropRight(2).takeRight(tradingAssetsSize / 2 - 1)).getOrElse(Acryl)
    )
    ...
    val work = orderType match {
      ...
      case GenOrderType.InvalidAmount =>
        ...
        val pair         = AssetPair(randomFrom(tradingAssets.takeRight(2)).get, Acryl)
      ...
      case GenOrderType.FakeSell =>
        ...
        val pair                      = AssetPair(randomFrom(tradingAssets.takeRight(2)).getOrElse(Acryl), Acryl)
        for {
