...
import com.acrylplatform.api.http.assets.{SignedIssueV2Request, SignedMassTransferRequest, SignedTransferV1Request}
import com.acrylplatform.common.state.ByteStr
import com.acrylplatform.crypto
import com.acrylplatform.it.api.{
  AssetBalance,
  Balance,
  MatcherResponse,
...
import com.acrylplatform.transaction.transfer.MassTransferTransaction.{ParsedTransfer, Transfer}
...
class ApiRequests(client: AsyncHttpClient) extends ScorexLogging {
  ...
  class Node(endpoint: String) {
    ...
    def balance(address: String, asset: Asset)(implicit tag: String): Future[Long] = asset match {
      case Acryl => to(endpoint).balance(address).map(_.balance)
