...
import com.acrylplatform.http.api_key
import com.acrylplatform.it.api.AsyncHttpApi.NodeAsyncHttpApi
import com.acrylplatform.it.sync.config.MatcherPriceAssetConfig
import com.acrylplatform.it.util.{GlobalTimer, TimerExt}
import com.acrylplatform.it.{Node, api}
...
object AsyncMatcherHttpApi extends Assertions {
  ...
  implicit class MatcherAsyncHttpApi(matcherNode: Node) extends NodeAsyncHttpApi(matcherNode) {
    ...
    def matcherApiEndpoint: URL = new URL(s"http://localhost:${matcherNode.nodeExternalPort(matcherNode.config.getInt("acryl.dex.port"))}")
