...
class MatcherMassOrdersTestSuite extends MatcherSuiteBase {
  ...
  "Create orders with statuses FILL, PARTIAL, CANCELLED, ACTIVE" - {
    ...
    val aliceSecondAcrylPair = AssetPair(IssuedAsset(ByteStr.decodeBase58(aliceSecondAsset).get), Acryl)
    ...
    val aliceOrderIdFill = node
      .placeOrder(alice, aliceSecondAcrylPair, OrderType.SELL, 3, Order.PriceConstant, matcherFee, orderVersion, 10.minutes)
      .message
      .id
    ...
    val aliceOrderToCancelId =
      node
        .placeOrder(alice, aliceSecondAcrylPair, OrderType.SELL, 3, Order.PriceConstant, matcherFee, orderVersion, 70.seconds)
        .message
        .id

    val aliceActiveOrderId = node
      .placeOrder(alice, aliceSecondAcrylPair, OrderType.SELL, 3, Order.PriceConstant + 100000000, matcherFee, orderVersion, 10.minutes)
      .message
      .id

    node.cancelOrder(alice, aliceSecondAcrylPair, aliceOrderToCancelId) // TODO: remove this line in DEX-160
    node.waitOrderStatus(aliceSecondAcrylPair, aliceOrderToCancelId, "Cancelled", 2.minutes)

    //Bob orders should partially fill one Alice order and fill another
    ordersRequestsGen(2, bob, aliceSecondAcrylPair, OrderType.BUY, 2)

    //check orders after filling
    node.waitOrderStatus(aliceSecondAcrylPair, alicePartialOrderId, "PartiallyFilled")

    orderStatus(alice, aliceSecondAcrylPair, aliceOrderIdFill, "Filled")
    orderStatus(alice, aliceSecondAcrylPair, alicePartialOrderId, "PartiallyFilled")
    ...
    "Mass orders creation with random lifetime. Active orders still in list" in {
      ...
      ordersRequestsGen(orderLimit + 1, alice, aliceAcrylPair, OrderType.SELL, 3)

      //wait for some orders cancelled
      Thread.sleep(5000)
      val bobsOrderIds = ordersRequestsGen(orderLimit + 1, bob, aliceAcrylPair, OrderType.BUY, 2)
      Thread.sleep(5000)
      ...
      node.waitOrderStatus(aliceSecondAcrylPair, aliceActiveOrderId, "Accepted")
      node.waitOrderStatus(aliceSecondAcrylPair, alicePartialOrderId, "PartiallyFilled")

      node.fullOrderHistory(bob).map(_.id) should equal(bobsOrderIds.drop(1).reverse)
      node.orderHistoryByPair(bob, aliceAcrylPair).map(_.id) should equal(bobsOrderIds.drop(1).reverse)
    }
    ...
    "check order history orders count after fill" in {
      ...
      val aliceOrderHistoryByPair = node.orderHistoryByPair(alice, aliceAcrylPair)
