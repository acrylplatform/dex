...
class MultipleMatchersTestSuite extends MatcherSuiteBase {
  private def configOverrides = ConfigFactory.parseString("""acryl.dex {
      |  price-assets = ["ACRYL"]
  ...
  private def matcher2NodeConfig =
    ConfigFactory
      ...
      |}
      |acryl.dex.events-queue.kafka.group = 1""".stripMargin)
  ...
  private val assetPair2                   = AssetPair(assetPair1.amountAsset, Acryl)
  private val assetPair3                   = AssetPair(assetPair1.priceAsset, Acryl)
