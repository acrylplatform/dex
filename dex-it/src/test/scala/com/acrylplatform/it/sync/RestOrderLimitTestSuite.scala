...
import com.acrylplatform.common.state.ByteStr.decodeBase58
...
class RestOrderLimitTestSuite extends MatcherSuiteBase {
  ...
  "Order History REST API methods should have limit for orders in response" in {
    ...
    val alicePair = AssetPair(IssuedAsset(decodeBase58(aliceAsset).get), Acryl)
    val bobPair   = AssetPair(IssuedAsset(decodeBase58(bobAsset).get), Acryl)

    info("'fullOrderHistory' and 'ordersByAddress' (activeOnly=false) must return no more 'rest-order-limit' orders")

    val active0 = node.placeOrder(alice, alicePair, SELL, 1, 15.acryl, matcherFee).message.id

    val active1    = node.placeOrder(alice, alicePair, SELL, 1, 10.acryl, matcherFee).message.id
    val partial1   = node.placeOrder(alice, alicePair, SELL, 2, 9.acryl, matcherFee).message.id
    val filled1    = node.placeOrder(alice, alicePair, SELL, 1, 8.acryl, matcherFee).message.id
    val cancelled1 = node.placeOrder(alice, alicePair, SELL, 1, 11.acryl, matcherFee).message.id
    val active2    = node.placeOrder(alice, bobPair, BUY, 1, 2.acryl, matcherFee).message.id
    val filled2    = node.placeOrder(alice, bobPair, BUY, 1, 4.acryl, matcherFee).message.id
    val partial2   = node.placeOrder(alice, bobPair, BUY, 2, 3.acryl, matcherFee).message.id
    val cancelled2 = node.placeOrder(alice, bobPair, BUY, 1, 2.acryl, matcherFee).message.id

    // orders for matching Alice's orders
    node.placeOrder(bob, alicePair, BUY, 1, 8.acryl, matcherFee).message.id // fill filled1
    node.placeOrder(bob, alicePair, BUY, 1, 9.acryl, matcherFee).message.id // part fill partial1
    node.placeOrder(bob, bobPair, SELL, 1, 4.acryl, matcherFee).message.id  // fill filled2
    node.placeOrder(bob, bobPair, SELL, 1, 3.acryl, matcherFee).message.id  // part fill partial2
    ...
    val active3 = node.placeOrder(alice, alicePair, SELL, 1, 10.acryl, matcherFee).message.id
    val active4 = node.placeOrder(alice, alicePair, SELL, 1, 10.acryl, matcherFee).message.id
    val active5 = node.placeOrder(alice, bobPair, BUY, 1, 2.acryl, matcherFee).message.id
    val active6 = node.placeOrder(alice, bobPair, BUY, 1, 2.acryl, matcherFee).message.id
    ...
    val active7  = node.placeOrder(alice, alicePair, SELL, 1, 9.acryl, matcherFee).message.id
    val active8  = node.placeOrder(alice, alicePair, SELL, 1, 10.acryl, matcherFee).message.id
    val active9  = node.placeOrder(alice, bobPair, BUY, 1, 1.acryl, matcherFee).message.id
    val active10 = node.placeOrder(alice, bobPair, BUY, 1, 1.acryl, matcherFee).message.id
    ...
    node.placeOrder(bob, bobPair, SELL, 1, 3.acryl, matcherFee).message.id   // fill partial2
    node.placeOrder(bob, bobPair, SELL, 2, 2.acryl, matcherFee).message.id   // fill active2, active5
    node.placeOrder(bob, alicePair, BUY, 2, 9.acryl, matcherFee).message.id  // fill partial1, active7
    node.placeOrder(bob, alicePair, BUY, 1, 10.acryl, matcherFee).message.id // fill active1
    ...
    val active11 = node.placeOrder(alice, alicePair, SELL, 1, 10.acryl, matcherFee).message.id
    val active12 = node.placeOrder(alice, alicePair, SELL, 1, 10.acryl, matcherFee).message.id
    val active13 = node.placeOrder(alice, alicePair, SELL, 1, 10.acryl, matcherFee).message.id
    val active14 = node.placeOrder(alice, alicePair, SELL, 1, 10.acryl, matcherFee).message.id
    val active15 = node.placeOrder(alice, alicePair, SELL, 1, 10.acryl, matcherFee).message.id
