...
import com.acrylplatform.transaction.assets.exchange.OrderType
...
class RoundingIssuesTestSuite extends MatcherSuiteBase {
  ...
  "should correctly fill an order with small amount" in {
    ...
    val counter   = node.prepareOrder(alice, acrylUsdPair, OrderType.BUY, 3100000000L, 238)
    val counterId = node.placeOrder(counter).message.id

    val submitted   = node.prepareOrder(bob, acrylUsdPair, OrderType.SELL, 425532L, 235)
    val submittedId = node.placeOrder(submitted).message.id

    val filledAmount = 420169L
    node.waitOrderStatusAndAmount(acrylUsdPair, submittedId, "Filled", Some(filledAmount), 1.minute)
    node.waitOrderStatusAndAmount(acrylUsdPair, counterId, "PartiallyFilled", Some(filledAmount), 1.minute)

    val tx = node.waitOrderInBlockchain(counterId).head
    node.cancelOrder(alice, acrylUsdPair, counterId)
  ...
  "should correctly fill 2 counter orders" in {
    val counter1 = node.prepareOrder(bob, acrylUsdPair, OrderType.SELL, 98333333L, 60L)
    node.placeOrder(counter1).message.id

    val counter2   = node.prepareOrder(bob, acrylUsdPair, OrderType.SELL, 100000000L, 70L)
    val counter2Id = node.placeOrder(counter2).message.id

    val submitted   = node.prepareOrder(alice, acrylUsdPair, OrderType.BUY, 100000000L, 1000L)
    val submittedId = node.placeOrder(submitted).message.id

    node.waitOrderStatusAndAmount(acrylUsdPair, counter2Id, "PartiallyFilled", Some(2857143L), 1.minute)
    node.waitOrderStatusAndAmount(acrylUsdPair, submittedId, "Filled", Some(99523810L), 1.minute)

    withClue("orderBook check") {
      val ob = node.orderBook(acrylUsdPair)
