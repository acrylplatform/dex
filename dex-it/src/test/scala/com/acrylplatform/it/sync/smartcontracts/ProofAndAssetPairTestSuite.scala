...
import com.acrylplatform.transaction.assets.exchange.{AssetPair, Order, OrderType, OrderV2}
    ...
    "positive scenarios of order placement" - {
      "set contracts with AssetPairs/all tx fields/true/one proof and then place order" - {
        for ((sc, i) <- Seq(sc1, sc3, sc4, sc5).zip(Seq(1, 3, 4, 5))) s"$i" in {
          ...
          node.waitOrderStatus(aliceAcrylPair, aliceOrd2, "Cancelled")
        }
      }
      ...
      "place order and then set contract on AssetPairs/true/all fields/one proof" - {
        for ((sc, i) <- Seq(sc1, sc3, sc4, sc5).zip(Seq(1, 3, 4, 5))) s"$i" in {
          log.debug(s"contract: $sc")
          val aliceOrd1 = node
            .placeOrder(alice, predefAssetPair, OrderType.BUY, 500, 2.acryl * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          node.waitOrderStatus(predefAssetPair, aliceOrd1, "Accepted", 1.minute)

          val aliceOrd2 = node
            .placeOrder(alice, aliceAcrylPair, OrderType.SELL, 500, 2.acryl * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          node.waitOrderStatus(aliceAcrylPair, aliceOrd2, "Accepted", 1.minute)
          ...
          val bobOrd1 = node
            .placeOrder(bob, predefAssetPair, OrderType.SELL, 500, 2.acryl * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          val bobOrd2 = node
            .placeOrder(bob, aliceAcrylPair, OrderType.BUY, 500, 2.acryl * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
      ...
      "place order and then set contract with many proofs" in {
        ...
        val transferTx = node.broadcastTransfer(alice, bob.address, 1000, 0.005.acryl, Some(aliceAsset), None)
        node.waitForTransaction(transferTx.id)

        for ((sc, i) <- Seq(sc5, sc6).zip(Seq(5, 6))) {
          markup(s"$i")

          for (assetP <- Seq(predefAssetPair, aliceAcrylPair)) {
            val currTime = System.currentTimeMillis()
          ...
          val bobOrd1 = node
            .placeOrder(bob, predefAssetPair, OrderType.SELL, 500, 2.acryl * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          val bobOrd2 = node
            .placeOrder(bob, aliceAcrylPair, OrderType.SELL, 500, 2.acryl * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
    ...
    "negative scenarios of order placement" - {
      ...
      "place order and then set contract" - {
        for ((contract, i) <- Seq(sc2, sc7, sc8, sc9).zip(Seq(2, 7, 8, 9))) s"$i" in {
          ...
          val aliceOrd1 = node
            .placeOrder(alice, predefAssetPair, OrderType.BUY, 100, 2.acryl * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          node.waitOrderStatus(predefAssetPair, aliceOrd1, "Accepted", 1.minute)
          ...
          val bobOrd1 = node
            .placeOrder(bob, predefAssetPair, OrderType.SELL, 100, 2.acryl * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id

          val bobOrd2 = node
            .placeOrder(bob, aliceAcrylPair, OrderType.BUY, 500, 2.acryl * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
