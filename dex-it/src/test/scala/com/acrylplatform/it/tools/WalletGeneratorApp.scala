package com.acrylplatform.it.tools
...
// dex-it/test:runMain com.acrylplatform.it.tools.WalletGeneratorApp dex/src/it/resources/template.conf dex/src/it/container/wallet.dat 0,1
/**
...
object WalletGeneratorApp extends App {
  ...
  val networkByte = {
    val x = config.getString("acryl.blockchain.custom.address-scheme-character")
    if (x.length != 1) throw new IllegalArgumentException(s"Invalid network byte in '$configFile': $x")
    x.head.toByte
  }

  val walletSettings = config.as[WalletSettings]("acryl.wallet")
