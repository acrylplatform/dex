...
//  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val settings: MatcherSettings = actualConfig.as[MatcherSettings]("acryl.dex")
    val db                        = openDB(settings.dataDir)
    val blockchainDb              = openDB(actualConfig.getString("acryl.db.directory"))
...
//        migrateOrderInfo(new ReadOnlyDB(blockchainDb, new ReadOptions()), db, settings.account, useFast, offset)
  ...
  class SK[A](suffix: String, extractor: Array[Byte] => Option[A]) {
    val keyBytes: Array[Byte] = ("matcher:" + suffix + ":").getBytes(UTF_8)
