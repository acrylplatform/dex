...
import com.acrylplatform.dex.AddressActor.GetOrderStatus
import com.acrylplatform.dex.AddressDirectory.{Envelope => Env}
import com.acrylplatform.dex.Matcher.StoreEvent
import com.acrylplatform.dex._
import com.acrylplatform.dex.error.{ErrorFormatterContext, MatcherError}
import com.acrylplatform.dex.market.MatcherActor.{ForceStartOrderBook, GetMarkets, GetSnapshotOffsets, MarketData, SnapshotOffsetsResponse}
...
import com.acrylplatform.dex.settings.{MatcherSettings, OrderRestrictionsSettings, formatValue}
...
case class MatcherApiRoute(assetPairBuilder: AssetPairBuilder,
  ...
  @Path("/settings/rates")
  @ApiOperation(value = "Asset rates", notes = "Get current rates of assets (asset cost in Acryl)", httpMethod = "GET")
  ...
  def upsertRate: Route = (path("settings" / "rates" / AssetPM) & put & withAuth) { a =>
    entity(as[Double]) { rate =>
      withAsset(a) { asset =>
        complete(
          if (asset == Acryl) StatusCodes.BadRequest -> wrapMessage("Rate for Acryl cannot be changed")
          else {
  ...
  def deleteRate: Route = (path("settings" / "rates" / AssetPM) & delete & withAuth) { a =>
    withAsset(a) { asset =>
      complete(
        if (asset == Acryl) StatusCodes.BadRequest -> wrapMessage("Rate for Acryl cannot be deleted")
        else {
          val assetStr = AssetPair.assetIdStr(asset)
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "amountAsset", value = "Amount Asset Id in Pair, or 'ACRYL'", dataType = "string", paramType = "path"),
      new ApiImplicitParam(name = "priceAsset", value = "Price Asset Id in Pair, or 'ACRYL'", dataType = "string", paramType = "path"),
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.acrylplatform.transaction.assets.exchange.Order"
      )
    ))
  ...
  @ApiImplicitParams(
    Array(
      ...
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.acrylplatform.dex.api.CancelOrderRequest"
      )
    ))
