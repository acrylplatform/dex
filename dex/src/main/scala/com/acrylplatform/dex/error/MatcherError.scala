...
import com.acrylplatform.dex.error.Class.{common => commonClass, _}
import com.acrylplatform.dex.error.Entity.{common => commonEntity, _}
import com.acrylplatform.dex.error.Implicits._
import com.acrylplatform.dex.settings.{DeviationsSettings, OrderRestrictionsSettings}
...
object MatcherError {
  implicit final class Ops(val self: MatcherError) extends AnyVal {
    def toJson(context: ErrorFormatterContext): JsObject = {
...
case class InvalidAsset(theAsset: String)
    extends MatcherError(asset, commonEntity, broken, e"The asset '${'assetId -> theAsset}' is wrong. It should be 'ACRYL' or a Base58 string")
