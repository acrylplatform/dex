...
import com.acrylplatform.dex.model.{BuyLimitOrder, LimitOrder, OrderValidator, SellLimitOrder}
import com.acrylplatform.dex.queue.{QueueEvent, QueueEventWithMeta}
import com.acrylplatform.dex.settings.OrderFeeSettings._
import com.acrylplatform.dex.settings.{AssetType, MatcherSettings}
...
import com.acrylplatform.transaction.assets.exchange.{AssetPair, Order, OrderType, OrderV3}
import com.acrylplatform.{NTPTime, crypto}
...
trait MatcherTestData extends NTPTime { _: Suite =>
  ...
  val assetPairGen = Gen.frequency((18, distinctPairGen), (1, assetIdGen(1).map(AssetPair(_, Acryl))), (1, assetIdGen(2).map(AssetPair(Acryl, _))))
  ...
  val matcherSettings = config.as[MatcherSettings]("acryl.dex")
  ...
  val maxAcrylAmountGen: Gen[Long] = Gen.choose(1, 100000000L * 100000000L)
  ...
  def buyGenerator(pair: AssetPair,
    ...
    for {
      ...
      matcherFee: Long <- matcherFee.map(Gen.const).getOrElse(maxAcrylAmountGen)
  ...
  val orderV3PairGenerator: Gen[((KeyPair, Order), (KeyPair, Order))] =
    for {
      ...
      matcherFeeBuy: Long  <- maxAcrylAmountGen
      matcherFeeSell: Long <- maxAcrylAmountGen
      arbitraryAsset       <- arbitraryAssetIdGen
      matcherFeeAssetId    <- Gen.oneOf(pair.amountAsset, pair.priceAsset, Acryl, arbitraryAsset)
    } yield {
      (
  ...
  val orderWithoutAcrylInPairAndWithFeeSettingsGenerator: Gen[(Order, KeyPair, OrderFeeSettings)] = {
    for {
      sender: KeyPair  <- accountGen
  ...
  private def correctOrderByFeeSettings(order: Order,
    ...
    val correctedOrder = (order.version, orderFeeSettings) match {
      ...
      case (_, DynamicSettings(baseFee)) =>
        order
          .updateMatcherFeeAssetId(matcherFeeAssetForDynamicSettings getOrElse Acryl)
          .updateFee(
