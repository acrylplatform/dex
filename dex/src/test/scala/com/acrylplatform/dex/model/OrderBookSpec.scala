...
import com.acrylplatform.dex.model.OrderBook.{LastTrade, Level, SideSnapshot, Snapshot}
import com.acrylplatform.dex.settings.MatchingRules
...
import com.acrylplatform.{NTPTime, NoShrink}
...
class OrderBookSpec extends FreeSpec with PropertyChecks with Matchers with MatcherTestData with NTPTime with NoShrink {
  val pair: AssetPair = AssetPair(Acryl, mkAssetId("BTC"))
  ...
  "buy small amount of pricey asset" in {
    val p = AssetPair(IssuedAsset(ByteStr("ACRYL".getBytes)), IssuedAsset(ByteStr("USD".getBytes)))
