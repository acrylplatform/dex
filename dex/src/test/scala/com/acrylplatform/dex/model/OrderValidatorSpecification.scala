...
import com.acrylplatform.dex.model.OrderValidator.Result
import com.acrylplatform.dex.settings.OrderFeeSettings.{DynamicSettings, FixedSettings, OrderFeeSettings, PercentSettings}
import com.acrylplatform.dex.settings.{AssetType, DeviationsSettings, OrderRestrictionsSettings}
import com.acrylplatform.dex.{MatcherTestData, RateCache}
import com.acrylplatform.settings.Constants
import com.acrylplatform.state.diffs.produce
import com.acrylplatform.state.{AssetDescription, Blockchain, LeaseBalance, Portfolio}
...
import com.acrylplatform.utils.randomBytes
import com.acrylplatform.{NoShrink, TestTime, WithDB}
...
class OrderValidatorSpecification
  ...
  private val pairAcrylBtc       = AssetPair(Acryl, wbtc)
  ...
  "OrderValidator" should {
    "allow buying ACRYL for BTC without balance for order fee" in asa() { v =>
      v shouldBe 'right
    }
    ...
    "reject new order" when {
      ...
      "matcherFee is too small (dynamic mode)" in forAll(orderV3WithPredefinedFeeAssetGenerator(Some(Acryl))) {
      ...
      "matcherFee is insufficient in case of scripted account or asset" in forAll(orderWithoutAcrylInPairAndWithFeeSettingsGenerator) {
      ...
      "sell order's price is out of deviation bounds (market aware)" in {
        val fixedAcrylFeeSettings = DynamicSettings(300000L)
        ...
        val assetPair2MarketStatus = new ConcurrentHashMap[AssetPair, MarketStatus]
        assetPair2MarketStatus.put(pairAcrylBtc, MarketStatus(None, Some(bestBid), Some(bestAsk)))
        val nonEmptyMarketStatus = assetPair2MarketStatus.get(pairAcrylBtc)
        ...
        val orderValidator = OrderValidator.marketAware(fixedAcrylFeeSettings, deviationSettings, Option(nonEmptyMarketStatus), rateCache) _
      ...
      "order's fee is out of deviation bounds (market aware)" in {
        ...
        val assetPair2MarketStatus = new ConcurrentHashMap[AssetPair, MarketStatus]
        assetPair2MarketStatus.put(pairAcrylBtc, MarketStatus(None, None, Some(bestAsk)))
        val nonEmptyMarketStatus = assetPair2MarketStatus.get(pairAcrylBtc)
      ...
      forAll(orderV3WithFeeSettingsGenerator) {
        case (order, orderFeeSettings) =>
          ...
          orderFeeSettings match {
            ...
            case _ =>
              // case _: FixedAcrylSettings => it's impossible to set script for Acryl
  ...
  private def newBuyOrder: Order =
    buy(pair = pairAcrylBtc, amount = 100 * Constants.UnitsInWave, price = 0.0022, matcherFee = Some((0.003 * Constants.UnitsInWave).toLong))
